...
class MerchantportallagomappServiceImpl(
  ...
  override def getRiskScore(
    ...
    ServerServiceCall { _ =>
      ...
      val getMerchantRisk = for {
        ...
        ifExist <- EitherT(
          merchantRiskScoreDetailRepo.checkRiskScoreExist(merchantId)
        )
        b <- EitherT(
  ...
  def getMerchantOnboardRiskData(
      merchantId: String
  ): Future[Either[String, MerchantRiskScoreResp]] = {
    val getAndUpdateQuery = for {
      onboardRiskScore <- EitherT(
        merchantOnboardRiskScore.getInitialRiskType(merchantId)
      )
      toRedis <- EitherT(
  ...
  override def addRiskType
    ...
    ServerServiceCall { riskJson =>
      val resp = for {
        toRedis <- EitherT(
          merchantRiskScoreDetailRepo.updateRiskScore(riskJson)
        )
  ...
  override def login = ServerServiceCall { (requestHeader, userLoginDetails) =>
    ...
    response map { case (res, auth) =>
      val header = ResponseHeader.Ok.withHeader(
        "Set-Cookie",
  ...
  override def logOut: ServiceCall[LogOutReq, Done] = ServerServiceCall { req =>
    val query = for {
      ...
      updateStatus <- EitherT(
        merchantLoginRepo.updateMerchantLoginStatus(req.userName)
      )
      del <- EitherT(redisUtility.deleteTokenFromRedis(req.userName))
    } yield (del)
    query.value.map {
      case Left(value) =>
        throw BadRequest(s"Failed to logOut merchant: ${req.userName}")
  ...
  override def getTxnSearchCriteriaList
      : ServiceCall[NotUsed, MerchantTxnSearchCriteria] =
    ServerServiceCall { _ =>
      Future(
        MerchantTxnSearchCriteria(
          "txnSearchCriteria",
