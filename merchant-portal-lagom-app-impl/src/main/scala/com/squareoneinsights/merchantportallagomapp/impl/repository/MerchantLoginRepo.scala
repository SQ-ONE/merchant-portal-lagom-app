...
class MerchantLoginRepo(val config: DatabaseConfig[JdbcProfile])(implicit
    ec: ExecutionContext
) extends Db
    with MerchantLoginTrait {
  ...
  def getUserByName(userName: String): Future[Either[String, MerchantLogin]] = {
    val query = usersTable.filter(_.merchantId === userName).result.asTry.map {
      merchantWithTry =>
        val fromTry =
